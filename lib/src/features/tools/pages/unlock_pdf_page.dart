...
class UnlockPdfPage extends ConsumerStatefulWidget {
  ...
  const UnlockPdfPage({super.key, this.initialFiles});
  ...
  Future<void> _processFiles() async {
    ...
    showDialog(
      ...
      builder: (context) => ToolActionDialog(
        title: _selectedFiles.length == 1 ? 'Unlocking PDF' : 'Batch Unlocking PDFs',
    ...
    for (final path in _selectedFiles) {
      ...
      final outputPath = p.join(dir, '${originalName}_unlocked.pdf');

      final result = await ToolsService.unlockPdf(path, outputPath, password);
      if (result != null) {
        successCount++;
    ...
    if (successCount == 0) {
      showDialog(
        ...
        builder: (context) => const ToolActionDialog(
          title: 'Error',
          message: 'Failed to unlock. Ensure the password is correct.',
          isLoading: false,
        ),
      );
    } else if (successCount == _selectedFiles.length && _selectedFiles.length == 1) {
      showDialog(
        context: context,
        builder: (context) => ToolActionDialog(
          title: 'Success',
      ...
      showDialog(
        ...
        builder: (context) => ToolActionDialog(
          ...
          message: 'Successfully unlocked $successCount out of ${_selectedFiles.length} files.\nSaved to: $dir',
          isLoading: false,
          outputPath: dir,
        ),
      );
    ...
    return Scaffold(
      ...
      floatingActionButton: _selectedFiles.isNotEmpty
          ? FloatingActionButton.extended(
              ...
              icon: const Icon(HugeIcons.strokeRoundedLockKey),
